          ...
          .items([
            ...
            S.listItem()
              ...
              .child(
                S.list()
                  .title('Filters')
                  .items([
                    S.listItem().title('All Albums').child(
            ...
            S.listItem()
              ...
              .child(
                S.list()
                  ...
                  S.listItem().title('English Pages').child(
                    S.documentTypeList('page')
                      .title('English Pages')
                      .filter('_type == "page" && language == "en"')
                  ),
                  S.listItem().title('French Pages').child(
                    S.documentTypeList('page')
                      .title('French Pages')
            ...
            S.listItem()
              ...
              ...S.documentTypeListItems().filter(
                (listItem) => !['siteSettings', 'album','page','post','category','pageList','author','translation.metadata'].includes(listItem.getId() ?? "")
              ),
          ])
  ...
  document: {
    actions: (prev: any, { schemaType }: any) => {
      ...
      if (['pageList','siteSettings'].includes(schemaType)) {
        ...
        const newActions = prev.filter((originalAction: any) =>
          originalAction.action !== "delete" && originalAction.action !== "duplicate" && originalAction.action !== "unpublish")
        return [...newActions]
      }
...
export const getDefaultDocumentNode = (S: { document: () => { (): any; new(): any; views: { (arg0: any[]): any; new(): any } }; view: { form: () => any; component: (arg0: any) => { (): any; new(): any; title: { (arg0: string): any; new(): any } } } }, { schemaType }: any) => {
