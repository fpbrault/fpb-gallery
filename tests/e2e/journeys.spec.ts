...
test("language switching preserves a dynamic album destination", async ({ page }) => {
  ...
  await expect(page).toHaveURL(new RegExp(`${frenchPath}import { expect, test, type Page } from "@playwright/test";
...
test("language switching preserves a dynamic album destination", async ({ page }) => {
  ...
  await expect(page).toHaveURL(new RegExp(`${englishPath}import { expect, test, type Page } from "@playwright/test";
