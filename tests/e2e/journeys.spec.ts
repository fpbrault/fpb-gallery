...
test("language switching preserves a dynamic album destination", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Switch language to French" }).click();

  const frenchPath = new URL(page.url()).pathname;
  expect(frenchPath).toBe(`/fr${englishPath}`);

  await page.getByLabel("open sidebar").click();
  await page.getByRole("link", { name: "Switch language to English" }).click();
  expect(new URL(page.url()).pathname).toBe(englishPath);
});
