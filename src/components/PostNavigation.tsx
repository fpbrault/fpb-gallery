...
function PostNavigationItem(props : any) {
  return (<div className='flex w-full nav-item indicator'>
    {props.data?.slug && <>
...
export function PostNavigation(props: any) {
  ...
  const relatedPostImages = {
    previous: props.data.previous?.coverImage ? getResizedImageSquare(props.data.previous?.coverImage, height, 80) : null,
    next: props.data.next?.coverImage ? getResizedImageSquare(props.data.next?.coverImage, height, 80) : null,
  };

  if (!props.data) {
    return null
  }
  return (<nav className='flex-col w-full max-w-2xl px-4 mx-auto my-8 space-y-6 sm:flex group lg:max-w-3xl sm:flex-row'>
    <span className='flex divider sm:hidden'></span>
    <PostNavigationItem data={props.data.previous} relatedPostImage={relatedPostImages.previous} label={"← Previous"}></PostNavigationItem>
    <span className='hidden my-2 sm:w-2/12 sm:flex divider divider-horizontal'></span>
    <PostNavigationItem data={props.data.next} relatedPostImage={relatedPostImages.next} label={"Next →"}></PostNavigationItem>
    <span className='flex divider sm:hidden'></span>
  </nav>);
}
