...
export const myPortableTextComponents = {
  ...
  types: {
    image: ({ value }: any) => {
      ...
      return (
        <div className=''>
          <Image width={500} height={500} className='rounded shadow-2xl object-fit ' unoptimized alt="" quality={75} {...value} src={src} /></div>
      );
    },
          ...
          <Link href={value.albumId}>
            <div className="absolute bottom-0 left-0 right-0 z-20 flex transition duration-300 ">
              <div className="max-w-full px-3 mx-auto mb-5 transition duration-300 rounded shadow bg-primary drop-shadow-xl backdrop-blur group-hover:bg-base-100">
                <div className="z-20 px-2 text-3xl text-center uppercase truncate align-middle transition duration-300 sm:text-xl md:text-2xl lg:text-3xl text-primary-content drop-shadow group-hover:text-primary">{value.albumName}</div></div>
            </div>
            <Image
              width={width}
              height={height}
